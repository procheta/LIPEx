...
num_samples = 1000
batch_size = 128
test_imgs_num = 3 # need to be defined
...
for var in var_list:
    add_noise(model.classifier[6], var)
    distort_p = Predictor(model)
    acc = accuracy(distort_p, test_data_set, true_idx_list)
    ...
    for image_name in image_list:
        ...
        model_LIPEx_TV_distance.append(TV(torch.from_numpy(distort_pred), distort_local_pred))
        
    model_LIPEx_TV_average = np.array(model_LIPEx_TV_distance).mean(axis=0)
