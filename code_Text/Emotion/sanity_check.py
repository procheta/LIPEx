...
for i in tqdm(range(len(test_text))):
    if len(set(test_text[i].split(' '))) > 25: 
...
model_TV_average_list, model_TV_std_list, LIPEx_TV_average_list, LIPEx_TV_std_list = [], [], [], []
...
for var in var_list:
    ...
    for idx in random_test_documents_idx:
        ...
        sample_data, sample_labels, sample_distances, sample_weights, features2use = explainer.sample_data_and_features(new_test.data[idx], c.predict, num_features=union_num, num_samples=1000)
        ...
        model_TV_distance.append(TV(torch.from_numpy(output), torch.from_numpy(distort_output)))
